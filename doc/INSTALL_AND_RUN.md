# Install and Run the Polkadot API Server
...
```bash
--pruning=archive --ws-port=9944 --ws-external --rpc-cors=all
```
...
```bash
cd polkadot_api_server
docker build -t simplyvc/polkadot_api_server:1.23.1 .
```
...
```bash
docker pull simplyvc/polkadot_api_server:1.23.1
```
